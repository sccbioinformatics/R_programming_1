...
Lets make the plot:
```{r fig.width=5, fig.height=4}
ggplot(data = df, aes(x = col1, y = col2)) + geom_line()
```

We can even add the points:
```{r fig.width=5, fig.height=4}
ggplot(data = df, aes(x = col1, y = col2)) + geom_line() + geom_point()
```

and change the colour of each:
```{r fig.width=5, fig.height=4}
ggplot(data = df, aes(x = col1, y = col2)) + geom_line(color = "red") + geom_point(color = "green")
```
